...
namespace Chess.Utils
{
    public static class PieceMoveHelpers
    {
        ...
        public static void AddCellsOnStraightLines(List<Move> possibleMoves, Cell position, GameState game, Player player)
        {
        ...
        public static void AddCellsOnDiagonalLines(List<Move> possibleMoves, Cell position, GameState game, Player player)
        {
            for (int i = 1; i < 7; i++)
            {
        ...
        public static void AddNeightboringCells(List<Move> possibleMoves, Cell position, GameState game, Player player)
        {
            AddIfValid(possibleMoves, position, new Cell(position.Row + 1, position.Col - 1), game, player);
            AddIfValid(possibleMoves, position, new Cell(position.Row + 1, position.Col), game, player);
            AddIfValid(possibleMoves, position, new Cell(position.Row + 1, position.Col + 1), game, player);
            AddIfValid(possibleMoves, position, new Cell(position.Row, position.Col - 1), game, player);
            AddIfValid(possibleMoves, position, new Cell(position.Row, position.Col + 1), game, player);
            AddIfValid(possibleMoves, position, new Cell(position.Row - 1, position.Col - 1), game, player);
            AddIfValid(possibleMoves, position, new Cell(position.Row - 1, position.Col), game, player);
            AddIfValid(possibleMoves, position, new Cell(position.Row - 1, position.Col + 1), game, player);
        ...
        public static bool AddIfValid(List<Move> possibleMoves, Cell fromCell, Cell toCell, GameState game, Player player)
        {
            return AddIfValid(possibleMoves, fromCell, toCell, game, player, (a, b) => true);
        }

        public static bool AddIfValid(List<Move> possibleMoves, Cell fromCell, Cell toCell, GameState game, Player player, Func<int, int, bool> condition)
        {
            ...
            if (possibleMoves.Any(x => x.FromCell == toCell))
            {
                return false;
            }
